...
export class UserModel {
  ...
  // Static method
  static makeSureUserExist = async (db: HermesChatDatabase, userId: string) => {
    await db.insert(users).values({ id: userId }).onConflictDoNothing();
  };

  static createUser = async (db: HermesChatDatabase, params: NewUser) => {
  ...
  static deleteUser = async (db: HermesChatDatabase, id: string) => {
    return db.delete(users).where(eq(users.id, id));
  };

  static findById = async (db: HermesChatDatabase, id: string) => {
    return db.query.users.findFirst({ where: eq(users.id, id) });
  };

  static findByEmail = async (db: HermesChatDatabase, email: string) => {
