...
const genProviderTable = (data: { desc?: string; id: string; name: string }) => {
  const title = genLink(data.name, urlJoin('https://hermes.chat/discover/provider', data.id));
  ...
  const newMd = updateReadme(
    ...
    [
      ...
      PROVIDER_LIST.slice(10, PROVIDER_LIST.length)
        ...
        .join('\n'),
      '</details>',
      `> 📊 Total providers: ${genLink(`<kbd>**${PROVIDER_LIST.length}**</kbd>`, 'https://hermes.chat/discover/providers')}`,
    ].join('\n\n'),
  );
