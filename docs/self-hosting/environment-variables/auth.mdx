---
title: Hermes Chat Authentication Service Environment Variables
description: >-
  Explore the essential environment variables for configuring authentication
  services in Hermes Chat, including OAuth SSO, NextAuth settings, and
...
Hermes Chat provides a complete authentication service capability when deployed. The following are the relevant environment variables. You can use these environment variables to easily define the identity verification services that need to be enabled in Hermes Chat.
...
<Callout>
  ...
  request](https://github.com/hermeslabs/hermes-chat/issues/new/choose) or Pull Request.
</Callout>
