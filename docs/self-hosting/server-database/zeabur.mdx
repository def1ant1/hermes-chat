---
title: Deploying Hermes Chat Database on Zeabur
description: >-
  Learn how to deploy Hermes Chat on Zeabur with ease. Follow the provided steps
  to set up your chat application seamlessly.
tags:
  - Deploy Hermes Chat
  - Zeabur Deployment
  - OpenAI API Key
...
# Deploying Hermes Chat Database on Zeabur
...
Here is the process for deploying the Hermes Chat server database version on Zeabur:

<Steps>
  ### Go to the template page on Zeabur

  Go to the [Hermes Chat Database template page](https://zeabur.com/templates/RRSPSD) on Zeabur and click on the "Deploy" button.
  ...
  - Hermes Chat Domain: A free subdomain with `.zeabur.app` suffix.
  ...
  After you fill all the required environment variables, select a region where you want to deploy your Hermes Chat Database and click on the "Deploy" button.
  ...
  Fill in those variables into your Hermes Chat service on Zeabur, here is a more detailed guide for [editing environment variables on Zeabur](https://zeabur.com/docs/deploy/variables).

  For detailed configuration of Logto, refer to [this document](/docs/self-hosting/advanced/auth/next-auth/logto).

  ### Access your Hermes Chat Instance

  Press on the `Hermes Chat-Database` and you can see the public domain you just created, click on it to access your Hermes Chat Database.
