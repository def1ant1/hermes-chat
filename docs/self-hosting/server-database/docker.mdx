---
title: Deploying Hermes Chat Database with Docker
description: >-
  Learn how to deploy the Hermes Chat server database version using Docker on
  Linux and local machines.
tags:
  - Hermes Chat
  - Docker
  - Database Deployment
...
<Callout type="warning">
  Due to the inability to expose `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` using Docker environment variables, you cannot use Clerk as an authentication service when deploying Hermes Chat using Docker / Docker Compose.
...
Here is the process for deploying the Hermes Chat server database version on a Linux server:
...
<Steps>
  ...
  <Callout type="info">
    ...
    component for Hermes Chat to implement RAG.
  </Callout>
  ...
  ### Create a file named `hermes-chat.env` to store environment variables:
  ...
  AUTH_AUTH0_ISSUER=https://hermes-chat-demo.us.auth0.com
  ...
  ### Start the hermes-chat-database Docker image

  ```sh
  docker run -it -d -p 3210:3210 --network pg --env-file hermes-chat.env --name hermes-chat-database hermeslabs/hermes-chat-database
  ```

  You can use the following command to check the logs:

  ```sh
  docker logs -f hermes-chat-database
  ```
...
The data version of Hermes Chat also supports direct use on a local Mac/Windows machine.
...
```shell
$ docker run -it -d --name hermes-chat-database -p 3210:3210 \
  ...
  -e AUTH_AUTH0_ISSUER=https://hermes-chat-demo.us.auth0.com \
  ...
  -e S3_BUCKET=hermes-chat \
  -e S3_PUBLIC_DOMAIN=https://s3-for-hermes-chat.your-domain.com \
  hermeslabs/hermes-chat-database
```
