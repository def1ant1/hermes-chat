---
title: 使用 Docker 部署 Hermes Chat 数据库
description: 详细步骤教你如何在 Docker 中部署 Hermes Chat 服务端数据库。
tags:
  - Docker
  - Hermes Chat
  - 数据库部署
...
<Callout type="info">
  本文已经假定你了解了 Hermes Chat 服务端数据库版本（下简称 DB
  版）的部署基本原理和流程，因此只包含核心环境变量配置的内容。如果你还不了解 Hermes Chat DB
...
<Callout type="warning">
  ...
  部署 Hermes Chat 时，你不能使用 Clerk 作为登录鉴权服务。
...
以下是在 Linux 服务器上部署 Hermes Chat DB 版的流程：
...
<Steps>
  ...
  <Callout type="info">
    pgvector 插件为 Postgres 提供了向量搜索的能力，是 Hermes Chat 实现 RAG 的重要构件之一。
  </Callout>
  ...
  ### 创建名为 `hermes-chat.env` 文件用于存放环境变量：
  ...
  ### 启动 hermes-chat-database docker 镜像
...
Hermes Chat 的 DB 版也支持直接在本地的 Mac/Windows 本地使用。
