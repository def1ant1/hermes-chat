---
title: Deploy Hermes Chat with database on Dokploy
description: >-
  Learn how to deploy Hermes Chat with database on Dokploy with ease, including:
...
This article will detail how to deploy the server database version of Hermes Chat.
...
In the server-side database, we need to configure the S3 storage service to store files. For detailed configuration instructions, please refer to the section [Configure S3 Storage Service](https://hermes.chat/docs/self-hosting/server-database/vercel#3-configure-s-3-storage-service) in the Vercel deployment guide。After the configuration is complete, you will obtain the following environment variables:
...
Obtain the three environment variables: `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`, and `CLERK_WEBHOOK_SECRET`. For detailed configuration steps for Clerk, please refer to the section [Configure Authentication Service](https://hermes.chat/docs/self-hosting/server-database/vercel#2-configure-authentication-service) in the Vercel deployment guide.
...
Enter the created database and set an unused port in External Credentials to allow external access; otherwise, Hermes Chat will not be able to connect to the database. You can view the Postgres database connection URL in External Host, as shown below:
...
## Deploy Hermes Chat on Dokploy.

Click "Create Service", select "Application", and create the Hermes Chat application.

![](https://github.com/user-attachments/assets/4cbbbcce-36be-48ff-bb0b-31607a0bba5c)

Enter the created Hermes Chat application, select the forked hermes-chat project and branch, and click Save to save.
...
After a successful deployment, bind your own domain to your Hermes Chat application and request a certificate on the Domains page.

![](https://github.com/user-attachments/assets/dd6bc4a4-3c20-4162-87fd-5cac57e5d7e7)

## Check if Hermes Chat is working properly.

Go to your Hermes Chat website, and if you click on the login button in the upper left corner and the login pop-up appears normally, it means you have configured it successfully. Enjoy it to the fullest!
