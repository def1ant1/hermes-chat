---
title: 在 Dokploy 上部署 Hermes Chat 的服务端数据库版本
description: 本文详细介绍如何在 Dokploy 中部署服务端数据库版 Hermes Chat，包括数据库配置、身份验证服务配置的设置步骤。
tags:
  - 服务端数据库
  - Postgres
  - Clerk
...
本文将详细介绍如何在 Dokploy 中部署服务端数据库版 Hermes Chat。
...
在服务端数据库中我们需要配置 S3 存储服务来存储文件，详细配置教程请参考 使用 Vercel 部署中 [配置 S3 储存服务](https://hermes.chat/zh/docs/self-hosting/server-database/vercel#%E4%B8%89%E3%80%81-%E9%85%8D%E7%BD%AE-s-3-%E5%AD%98%E5%82%A8%E6%9C%8D%E5%8A%A1)。配置完成后你将获得以下环境变量：
...
获取 `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` 、`CLERK_SECRET_KEY` 、`CLERK_WEBHOOK_SECRET` 这三个环境变量，Clerk 的详细配置流程请参考 使用 Vercel 部署中 [配置身份验证服务](https://hermes.chat/zh/docs/self-hosting/server-database/vercel#二、-配置身份验证服务)
...
进入创建的数据库，在 External Credentials 设置一个未被占用的端口，使其能能通过外部访问，否则 Hermes Chat 将无法连接到该数据库。你可以在 External Host 查看 Postgres 数据库连接 URL ，如下：
...
## 在 Dokploy 上部署 Hermes Chat

点击 Create Service 选择 Application，创建 Hermes Chat 应用

![](https://github.com/user-attachments/assets/4cbbbcce-36be-48ff-bb0b-31607a0bba5c)

进入创建的 Hermes Chat 应用，选择你 fork 的 hermes-chat 项目及分支，点击 Save 保存
...
部署成功后在 Domains 页面，为你的 Hermes Chat 应用绑定自己的域名并申请证书。

![](https://github.com/user-attachments/assets/dd6bc4a4-3c20-4162-87fd-5cac57e5d7e7)

## 验证 Hermes Chat 是否正常工作

进入你的 Hermes Chat 网址，如果你点击左上角登录，可以正常显示登录弹窗，那么说明你已经配置成功了，尽情享用吧～
