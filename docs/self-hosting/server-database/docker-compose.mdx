---
title: Deploying Hermes Chat with Docker Compose
description: >-
  Learn how to deploy the Hermes Chat service using Docker Compose, including
...
# Deploying Hermes Chat Server Database Version with Docker Compose
...
Execute the following commands to set up the deployment environment; the directory `hermes-chat-db` will be used to store your configuration files and subsequent database files.
...
<Steps>
  ...
  <Callout type="warning">
    Please log in to Hermes Chat using the user account; the administrator account is only for managing
    Casdoor.
  </Callout>
  ...
  Visit your Hermes Chat service at [http://localhost:3210](http://localhost:3210). The account credentials for the application can be found in the report from step `2`.
</Steps>
...
<Steps>
  ...
  You can access your Hermes Chat service at `http://your_server_ip:3210`. The account credentials for the application can be found in the report from step `2`.

  <Callout type="warning">
    If your service can accessed via the public network,
    we strongly recommend disabling the registration,
    refer to the [documentation](https://hermes.chat/docs/self-hosting/advanced/auth/next-auth/casdoor)
...
<Steps>
  ...
  <Callout type="important">
    ...
    Read more at [https://github.com/hermeslabs/hermes-chat/discussions/5986](https://github.com/hermeslabs/hermes-chat/discussions/5986)
  </Callout>
  ...
  - Domain setup for the Hermes Chat service: `lobe.example.com`
  ...
  You can access your Hermes Chat service via `https://lobe.example.com`. The account credentials for the application can be found in the report from step `3`.
...
This section mainly introduces the configurations that need to be modified to customize the deployment of the Hermes Chat service in different network environments. Before starting, you can download the [Docker Compose configuration file](https://raw.githubusercontent.com/hermeslabs/hermes-chat/HEAD/docker-compose/local/docker-compose.yml) and the [environment variable configuration file](https://raw.githubusercontent.com/hermeslabs/hermes-chat/refs/heads/main/docker-compose/local/.env.example).

```sh
curl -O https://raw.githubusercontent.com/hermeslabs/hermes-chat/HEAD/docker-compose/local/docker-compose.yml
curl -O https://raw.githubusercontent.com/hermeslabs/hermes-chat/HEAD/docker-compose/local/.env.example
mv .env.example .env
```
...
- Hermes Chat requires communication with Casdoor, so you need to configure Casdoor's Issuer.

```env
AUTH_CASDOOR_ISSUER=https://auth.example.com
```

This configuration will affect Hermes Chat's login authentication service, and you need to ensure that the URL of the Casdoor service is correct. You can find common manifestations and solutions for errors in this configuration in the [FAQ](#faq).

- Additionally, you need to allow the callback URL in Casdoor to point to the Hermes Chat address:
...
- Hermes Chat needs to provide a public access URL for object files for the LLM service provider, hence you need to configure MinIO's Endpoint.
...
```log
hermes-chat      | [auth][error] r3: "response" is not a conform Authorization Server Metadata response (unexpected HTTP status code)
```
...
```log
hermes-chat      | [auth][error] TypeError: fetch failed
```

Cause: Hermes Chat cannot access the authentication service.

Solutions:

- Check whether your authentication service is running properly and whether Hermes Chat's network can reach the authentication service.

- A straightforward troubleshooting method is to use the `curl` command in the Hermes Chat container terminal to access your authentication service at `https://auth.example.com/.well-known/openid-configuration`. If JSON format data is returned, it indicates your authentication service is functioning correctly.

````markdown
## Extended Configuration

To enhance your Hermes Chat service, you can perform the following extended configurations according to your needs.
...
- `lobe.example.com`: This is your Hermes Chat service domain, which needs to reverse proxy to the Hermes Chat service port, default is `3210`.
...
```sh
bash <(curl -fsSL https://raw.githubusercontent.com/hermeslabs/hermes-chat/HEAD/docker-compose/local/setup.sh) -f -l zh_CN
docker compose up -d
```
...
1. In `User Management -> Organizations`, add a new organization with the name and display name `Hermes Users`. Keep the rest as default.
2. In `Authentication -> Apps`, add a new application.

- Name and display name should be `Hermes Chat`.
- Organization should be `Hermes Users`.
...
<Callout type="warning">
  Please remember to configure the corresponding S3 service provider's CORS settings to ensure that Hermes Chat can access the S3 service correctly.
...
3. Restart the Hermes Chat service:

   ```sh
   docker compose up -d
   ```

At this point, you have successfully deployed the Hermes Chat database version, and you can access your Hermes Chat service at `https://lobe.example.com`.
...
services:
  ...
  lobe:
    image: hermeslabs/hermes-chat-database
    container_name: hermes-chat-database
    network_mode: 'service:network-service'
    depends_on:
      postgresql:
        condition: service_healthy
...
[docker-pulls-shield]: https://img.shields.io/docker/pulls/hermeslabs/hermes-chat-database?color=45cc11&labelColor=black&style=flat-square
