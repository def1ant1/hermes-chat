---
title: 通过 Docker Compose 部署 Hermes Chat
description: 学习如何使用 Docker Compose 部署 Hermes Chat 服务，包括各种服务的配置教程。
tags:
  - Docker Compose
  - Hermes Chat
  - Docker 容器
...
# 使用 Docker Compose 部署 Hermes Chat 服务端数据库版本
...
执行以下命令初始化部署环境，目录 `hermes-chat-db` 将用于存放你的配置文件和后续的数据库文件。
...
<Steps>
  ...
  <Callout type="warning">
    请使用用户账号登录 Hermes Chat，管理员账号仅用于管理 Casdoor。
  </Callout>
  ...
  通过 [http://localhost:3210](http://localhost:3210) 访问你的 Hermes Chat 服务。应用的账号密码在步骤`2`的报告中。
</Steps>
...
<Steps>
  ...
  你可以通过 `http://your_server_ip:3210` 访问你的 Hermes Chat 服务。应用的账号密码在步骤`2`的报告中。

  <Callout type="warning">
    请注意，如果你的服务能够被公网访问，我们强烈建议你参考 [文档](https://hermes.chat/docs/self-hosting/advanced/auth/next-auth/casdoor) 关闭注册功能。
  </Callout>
</Steps>
...
<Steps>
  ...
  <Callout type="important">
    ...
    详情请见 [https://github.com/hermeslabs/hermes-chat/discussions/5986](https://github.com/hermeslabs/hermes-chat/discussions/5986)
  </Callout>
  ...
  - Hermes Chat 服务的域名设置：`lobe.example.com`
  ...
  你可以通过 `https://lobe.example.com` 访问你的 Hermes Chat 服务。应用的账号密码在步骤`3`的报告中。
...
该章节主要为你介绍在不同的网络环境下自定义部署 Hermes Chat 服务必须要修改的配置。在开始前，你可以先下载 [Docker Compose 配置文件](https://raw.githubusercontent.com/hermeslabs/hermes-chat/HEAD/docker-compose/local/docker-compose.yml) 以及 [环境变量配置文件](https://raw.githubusercontent.com/hermeslabs/hermes-chat/HEAD/docker-compose/local/.env.zh-CN.example)。

```sh
curl -O https://raw.githubusercontent.com/hermeslabs/hermes-chat/HEAD/docker-compose/local/docker-compose.yml
curl -O https://raw.githubusercontent.com/hermeslabs/hermes-chat/HEAD/docker-compose/local/.env.zh-CN.example
...
一般来讲，想要完整的运行 Hermes Chat 数据库版本，你需要至少拥有如下四个服务

- Hermes Chat 数据库版本自身
- 带有 PGVector 插件的 PostgreSQL 数据库
- 支持 S3 协议的对象存储服务
- 受 Hermes Chat 支持的 SSO 登录鉴权服务

这些服务可以通过自建或者在线云服务组合搭配，以满足不同层次的部署需求。本文中，我们提供了完全基于开源自建服务的 Docker Compose 配置，你可以直接使用这份配置文件来启动 Hermes Chat 数据库版本，也可以对之进行修改以适应你的需求。
...
- Hermes Chat 需要与 Casdoor 通讯，因此你需要配置 Casdoor 的 Issuer 。

```env
AUTH_CASDOOR_ISSUER=https://auth.example.com
```

该配置会影响 Hermes Chat 的登录鉴权服务，你需要确保 Casdoor 服务的地址正确。你可以在 [常见问题](#常见问题) 中找到该配置错误的常见现象及解决方案。

- 同时，你也需要在 Casdoor 中允许回调地址为 Hermes Chat 的地址：
...
- Hermes Chat 需要为 LLM 服务提供商提供文件对象的公网访问地址，因此你需要配置 MinIO 的 Endpoint 。
...
成因：Hermes Chat 无法访问鉴权服务。

解决方案：

- 请检查你的鉴权服务是否正常运行，以及 Hermes Chat 所在的网络是否能够访问到鉴权服务。

- 一个直接的排查方式，你可以在 Hermes Chat 容器的终端中，使用 `curl` 命令访问你的鉴权服务 `https://auth.example.com/.well-known/openid-configuration`，如果返回了 JSON 格式的数据，则说明你的鉴权服务正常运行。

## 拓展配置

为了完善你的 Hermes Chat 服务，你可以根据你的需求进行以下拓展配置。
...
- `lobe.example.com`：为你的 Hermes Chat 服务端域名，需要反向代理到 Hermes Chat 服务端口，默认为 `3210`
...
1. 在 `用户管理 -> 组织` 中，添加一个新的组织。名称与显示名称为 `Hermes Users`。其余保持默认即可。
2. 在 `身份认证 -> 应用` 中，添加一个新的应用。

- 名称与显示名称为 `Hermes Chat`。
- 组织为 `Hermes Users`。
...
<Callout type="warning">
  请记得注意配置对应 S3 服务商的 CORS 跨域配置，以确保 Hermes Chat 能够正常访问 S3 服务。
...
至此，你已经成功部署了 Hermes Chat 数据库版本，你可以通过 `https://lobe.example.com` 访问你的 Hermes Chat 服务。
