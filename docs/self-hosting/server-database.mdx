---
title: Deploying Server-Side Database for Hermes Chat
description: Learn how to deploy Hermes Chat's server-side database using Postgres.
tags:
  - Hermes Chat
...
Hermes Chat defaults to using a client-side database (IndexedDB) but also supports deploying a server-side database. Hermes Chat uses Postgres as the backend storage database.
...
This guide will introduce the process and principles of deploying the server-side database version of Hermes Chat on any platform from a framework perspective, so you can understand both the what and the why, and then deploy according to your specific needs.
...
For the server-side database version of Hermes Chat, a normal deployment process typically involves configuring three modules:
...
Since we support file-based conversations/knowledge base conversations, we need to install the `pgvector` plugin for Postgres. This plugin provides vector search capabilities and is a key component for Hermes Chat to implement RAG.

<Steps>
  ### `NEXT_PUBLIC_SERVICE_MODE`

  Hermes Chat supports both client-side and server-side databases, so we provide an environment variable for switching modes, which is `NEXT_PUBLIC_SERVICE_MODE`, with a default value of `client`.

  For server-side database deployment scenarios, you need to set `NEXT_PUBLIC_SERVICE_MODE` to `server`.

  <Callout type={'info'}>
    In the official `hermes-chat-database` Docker image, this environment variable is already set to
  ...
  - In our provided Docker image `hermes-chat-database`, `DATABASE_DRIVER` defaults to `node`.
...
[Clerk](https://clerk.com?utm_source=hermeslabs\&utm_medium=docs) is an authentication SaaS service that provides out-of-the-box authentication capabilities with high productization, low integration costs, and a great user experience. For those who offer SaaS products, Clerk is a good choice. Our official [Hermes Chat Cloud](https://hermes-chat.com) uses Clerk as the authentication service.
...
The above is a detailed explanation of configuring Hermes Chat with a server-side database. You can configure it according to your actual situation and then choose a deployment platform that suits you to start deployment:
