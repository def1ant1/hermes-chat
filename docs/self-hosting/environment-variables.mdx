---
title: Hermes Chat Environment Variables - Customizing Guide
description: >-
  Learn how to customize Hermes Chat configuration using environment variables
...
Hermes Chat provides some additional configuration options when deployed, which can be customized using environment variables.
...
If you need to override `NEXT_PUBLIC` environment variables, you can build a custom Docker image using GitHub Actions without forking the entire Hermes Chat repository. Here's a guide on how to do this:
...
env:
  REGISTRY: ghcr.io
  IMAGE_NAME: ${{ github.repository_owner }}/hermes-chat-database # Name of your image
  ...
  build-and-push:
    ...
    steps:
      - name: Checkout repository
        ...
        with:
          repository: hermeslabs/hermes-chat
...
6. To build your custom image, go to the "Actions" tab in your GitHub repository and manually trigger the "Build Custom Hermes Chat Image" workflow.

This approach allows you to create a custom build with your desired `NEXT_PUBLIC` variables without maintaining a full fork of the Hermes Chat repository. You can trigger a new build whenever you need to update your custom image.
