---
title: Integrating Hermes Chat with Ollama for Enhanced Language Models
description: >-
  Learn how to configure and deploy Hermes Chat to leverage Ollama's powerful
  language models locally. Follow the guide to run Ollama and Hermes Chat on
  your system.
tags:
  - Ollama integration
...
Ollama is a powerful framework for running large language models (LLMs) locally, supporting various language models including Llama 2, Mistral, and more. Now, Hermes Chat supports integration with Ollama, meaning you can easily use the language models provided by Ollama to enhance your application within Hermes Chat.

This document will guide you on how to configure and deploy Hermes Chat to use Ollama:
...
## Running Hermes Chat Locally

Assuming you have already started the Ollama service locally on port `11434`. Run the following Docker command to start Hermes Chat locally:

```bash
docker run -d -p 3210:3210 -e OLLAMA_PROXY_URL=http://host.docker.internal:11434 hermeslabs/hermes-chat
```

Now, you can use Hermes Chat to converse with the local LLM.

For more information on using Ollama in Hermes Chat, please refer to [Ollama Usage](/docs/usage/providers/ollama).
