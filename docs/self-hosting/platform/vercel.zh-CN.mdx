...
> - **Compatibility window:** Legacy `@hermeslabs/*` and `@hermeslabs/*` packages receive hotfix updates until 2025-09-30 so you can stage blue/green rollouts or orchestrate emergency downgrades.
...
> - **Breaking-change considerations:** Update any Vercel integration still targeting the `hermeslabs/hermes-chat` repository to `hermeslabs/hermes-chat` before the compatibility window closes; otherwise automated deploys will fail once the legacy repo is archived.
