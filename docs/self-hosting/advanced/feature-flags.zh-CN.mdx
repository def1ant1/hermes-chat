---
title: Hermes Chat 特性标志环境变量配置指南
description: 了解如何使用环境变量自定义 Hermes Chat 的特性标志，包括控制否启用某个功能、或者根据需要对特定用户群体或环境启用或禁用功能。
tags:
  - Hermes Chat
  - 环境变量
  - 配置指南
...
除了基础的环境变量配置外，Hermes Chat 还提供了一些特性标志（Feature Flags），用于控制是否全局启用某个功能，或者根据需要对特定用户群体或环境启用或禁用功能。
...
你可以随时检查 [featureFlags](https://github.com/hermeslabs/hermes-chat/blob/main/src/config/featureFlags/schema.ts) 以获取最新的特性标志列表。
