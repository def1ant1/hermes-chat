...
title: >-
  ...
  Information
description: >-
  Learn how to configure the SearXNG online search functionality for Hermes
  Chat, enabling AI to access the latest web information.
tags:
  - Online Search
  - SearXNG
  - Web Information
...
You can find publicly available SearXNG instances in the [SearXNG instance list](https://searx.space/). Choose an instance that is fast and reliable, and then configure its URL in Hermes Chat.

> Note that the `searxng` you use must have `json` output enabled; otherwise, the `hermes-chat` call will result in an error. If self-hosting, find the `searxng` configuration file and add `json` as shown below.
...
1. Restart the Hermes Chat service.
...
- [Hermes Chat Online Search RFC Discussion](https://github.com/hermeslabs/hermes-chat/discussions/6447)
