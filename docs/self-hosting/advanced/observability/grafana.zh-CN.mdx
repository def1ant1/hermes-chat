...
description: >-
  使用 Grafana、Prometheus 指标和 Tempo 链路追踪，监控和分析你的 Hermes Chat
...
Hermes Chat 支持通过开源工具实现自托管部署的高级可观测性：
...
2. 使用如下命令启动服务：
   ...
   这将会启动 Grafana、Prometheus、Tempo、otel-collector 以及 Hermes Chat、Casdoor、Minio 等高级服务。
...
- 检查 Prometheus 和 Tempo 是否正常采集 Hermes Chat 及 otel-collector 的数据。

## 相关链接

- [Langfuse 可观测性](https://hermes.chat/zh/docs/self-hosting/advanced/observability/langfuse)
- [自托管总览](https://hermes.chat/zh/docs/self-hosting/start)
