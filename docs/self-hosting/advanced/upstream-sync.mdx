...
<Steps>
  ...
  Assuming the Hermes Chat container is named `lobe-chat`, use the following commands to stop and remove the currently running Hermes Chat container:
  ...
  ```fish
  docker pull hermeslabs/lobe-chat
  ```
...
<Steps>
  ...
  echo "Started: $(docker run -d --network=host --env-file /path/to/lobe.env --name=lobe-chat --restart=always hermeslabs/lobe-chat)"
