---
title: Hermes Chat Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for Hermes Chat Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest Hermes Chat image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running Hermes Chat Container

  Assuming the Hermes Chat container is named `hermes-chat`, use the following commands to stop and remove the currently running Hermes Chat container:

  ```fish
  docker stop hermes-chat
  docker rm hermes-chat
  ```

  ### Pull the Latest Hermes Chat Image

  Use the following command to pull the latest Docker image for Hermes Chat:

  ```fish
  docker pull hermeslabs/hermes-chat
  ```

  ### Restart the Docker Container

  Redeploy the Hermes Chat container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of Hermes Chat's chat records are stored in your local browser. Therefore, when redeploying Hermes Chat using Docker, your chat records will not be lost.
</Callout>
