---
title: 与 Hermes Chat 桌面端自动同步
description: 配置 Hermes Chat 桌面端，获取端同步体验
tags:
  - Hermes Chat
  - 桌面端
  - 自托管
...
# 与 Hermes Chat 桌面端自动同步

Hermes Chat 桌面端为用户提供了更好的使用体验，同时也扩展了 Hermes Chat 的能力边界，允许用户在离线环境下使用 Hermes Chat。本文档将指导您如何配置 Hermes Chat 桌面端以及如何将其与自托管实例连接。

## 系统要求

Hermes Chat 桌面端使用 Electron 构建，支持以下操作系统：
...
Hermes Chat 桌面端的版本发布遵循以下逻辑：

- **Beta 版本**：所有合并到 main 分支并自动发布版本，都会自动创建为 Hermes Chat 桌面端的 Beta 版本
...
Hermes Chat 桌面端可以与您自托管的 Hermes Chat 实例连接，以便您可以在桌面端使用您的自托管配置。
...
如果您使用一键部署方式（如 Vercel、Railway 等平台）部署 Hermes Chat，您需要：
...
1. 打开 Hermes Chat 桌面端
2. 点击右上角 wifi 图标
3. 输入您的自托管实例地址（例如：`https://your-hermes-chat-instance.com`）
4. 点击 "连接" 按钮
...
- 文件名示例：
  - Apple Silicon: `Hermes Labs-Beta-1.83.6-arm64.dmg` 或 `Hermes Labs-Beta-1.83.6-arm64-mac.zip`
  - Intel: `Hermes Labs-Beta-1.83.6-x64.dmg` 或 `Hermes Labs-Beta-1.83.6-mac.zip`
