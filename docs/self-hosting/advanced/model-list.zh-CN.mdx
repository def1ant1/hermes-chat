---
title: Hermes Chat 自定义模型服务商模型列表及扩展能力配置
description: 了解如何在 Hermes Chat 中自定义模型列表以及扩展能力配置的基本语法和规则。
tags:
  - Hermes Chat
  - 自定义模型列表
  - 扩展能力配置
...
Hermes Chat 支持在部署时自定义模型列表，详情请参考 [模型提供商](/zh/docs/self-hosting/environment-variables/model-provider) 。
