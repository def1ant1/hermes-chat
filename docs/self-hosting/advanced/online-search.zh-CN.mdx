...
description: 了解如何为 Hermes Chat 配置 SearXNG 联网搜索功能，使 AI 能够获取最新的网络信息。
tags:
  - 联网搜索
  - SearXNG
  - 网络信息
...
Hermes Chat 支持为 AI 配置**联网搜索功能**，使其能够实时获取互联网信息，从而提供更准确、最新的回答。联网搜索支持多个搜索引擎提供商，包括 [SearXNG](https://github.com/searxng/searxng)、[Search1API](https://www.search1api.com)、[Google](https://programmablesearchengine.google.com)、[Brave](https://brave.com/search/api) 等。

<Callout type="info">
  联网搜索可以让 AI 获取时效性内容，如最新新闻、技术动态或产品信息。你可以使用开源的 SearXNG 自行部署，也可以选择集成主流搜索引擎服务，如 Search1API、Google、Brave 等，根据你的使用场景自由组合。
</Callout>

通过设置搜索服务环境变量 `SEARCH_PROVIDERS` 和对应的 API Key，Hermes Chat 将在多个搜索源中查询并返回结果。你还可以搭配配置爬虫服务环境变量 `CRAWLER_IMPLS`（如 `browserless`、`firecrawl`、`tavily` 等）以提取网页内容，实现搜索 + 阅读的增强能力。
...
您可以在 [SearXNG 实例列表](https://searx.space/) 中找到公开可用的 SearXNG 实例。选择一个响应速度快、可靠性高的实例，然后将其 URL 配置到 Hermes Chat 中。

> 注意，使用的 `searxng` 必须开启 `json` 输出，否则 `hermes-chat` 调用会报错。如果是自托管，类似下面这样，找到 `searxng` 的配置文件，追加 `json` 即可。
...
- [Hermes Chat 联网搜索 RFC 讨论](https://github.com/hermeslabs/hermes-chat/discussions/6447)
