---
title: Configuring Authelia Authentication Service for Hermes Chat
description: >-
  Learn how to configure Authelia authentication service in Hermes Chat,
  including creating a provider, configuring environment variables, and
  deploying Hermes Chat. Detailed steps and necessary environment variable
  settings.
tags:
...
<Steps>
  ...
  Make sure to replace secret and `redirect_urls` with your own values. Note! The secret configured in Authelia is ciphertext, i.e., a salted hash value. Its corresponding plaintext needs to be filled in Hermes Chat later.
  ...
  | `AUTH_AUTHELIA_ID`        | Required | The id just configured in Authelia, example value is hermes-chat                                                                                                                                                                                          |
