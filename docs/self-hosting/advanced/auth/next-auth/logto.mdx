---
title: Configuring Logto Authentication Service in Hermes Chat
description: >-
  Learn how to configure Logto authentication service in Hermes Chat, including
...
<Callout type={'tip'}>
  ...
  with the Hermes Chat database version. In this case, Hermes Chat can share the same Postgres instance
  with it.
</Callout>

## Logto Configuration Process

The following assumes your Hermes Chat database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  Configure the Logto Webhook so that Hermes Chat can receive notifications when user information is updated.
  ...
  Set the obtained `Client ID` and `Client Secret` as `AUTH_LOGTO_ID` and `AUTH_LOGTO_SECRET` in the Hermes Chat environment variables.

  Configure `AUTH_LOGTO_ISSUER` in the Hermes Chat environment variables as follows:
  ...
  | `NEXT_AUTH_SSO_PROVIDERS`   | Required | Select the single sign-on provider for Hermes Chat. For Logto, enter `logto`.                                                                                                          |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate via Logto and use Hermes Chat.
</Callout>
