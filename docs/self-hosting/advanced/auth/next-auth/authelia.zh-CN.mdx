---
title: 在 Hermes Chat 中配置 Authelia 身份验证服务
description: >-
  学习如何在 Hermes Chat 中配置 Authelia 身份验证服务，包括创建提供程序、配置环境变量和部署 Hermes
  Chat。详细步骤和必要环境变量设置。
tags:
  - Authelia
  - 身份验证
...
<Steps>
  ...
  我们现在默认您已经了解了如何使用 Authelia。假设您的 Hermes Chat 实例部署在 `https://lobe.example.com/` 中。注意，目前只有 `localhost` 支持 HTTP 访问，其他域名需要启用 TLS，否则 Authelia 默认将主动中断身份认证。

  现在，我们打开 Authelia 实例的配置文件进行编辑：

  在 `identity_providers`-> `oidc` 下新增一个 `hermes-chat` 的项目：
  ...
  | `AUTH_AUTHELIA_ID`        | 必选 | 刚刚在 Authelia 配置的 `id`，示例值是 `hermes-chat`                                                         |
...
<Callout type={'info'}>
  部署成功后，用户将可以使用 Authelia 中配置的用户通过身份认证并使用 Hermes Chat。
</Callout>
