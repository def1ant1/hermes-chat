---
title: Configuring Keycloak Authentication Service in Hermes Chat
description: >-
  Learn how to configure the Keycloak authentication service in Hermes Chat,
...
# Configuring Keycloak Authentication Service in Hermes Chat
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with Hermes Chat via Docker
...
- Your Hermes Chat database version IP/port is `http://LOBECHAT_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your Hermes Chat database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "Hermes Chat", then click "Create"

  2. Create a Client

     - Select "Clients" from the left menu, then click "Create client"
     - Fill in the following information:
       - Client ID: `hermes-chat`
     ...
     - On the "Login settings" page:
       ...
       - Web origins: Add your Hermes Chat domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the Hermes Chat environment variables.

  Configure the Hermes Chat environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/Hermes Chat` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/Hermes Chat` for privately deployed Keycloak on a local network
  - `https://lobe-auth-api.example.com/realms/Hermes Chat` for Keycloak deployed in a public environment
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for Hermes Chat. For Keycloak, fill in `keycloak`.                                                                                                    |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use Hermes Chat.
</Callout>
