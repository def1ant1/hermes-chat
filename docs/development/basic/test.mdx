...
Hermes Chat's testing strategy includes unit testing and end-to-end (E2E) testing. Below are detailed explanations of each type of testing:
...
Currently, there is no integrated end-to-end testing in Hermes Chat. We will gradually introduce end-to-end testing in subsequent iterations.

## Development Testing

### 1. Unit Testing

Unit testing is conducted on the smallest testable units in the application, usually functions, components, or modules. In Hermes Chat, we use [vitest][vitest-url] for unit testing.
...
> \[!TIP]
> Keep Discover surfaces green by running the targeted suites before opening a
> pull request:
>
> - `bunx vitest run --silent=passed-only src/app/discover/__tests__/discoverMetadata.test.tsx`
...
> - `bunx vitest run --silent='passed-only' 'src/app/**/__tests__/AuthErrorPage.test.tsx'`
>   ensures NextAuth errors surface the Hermes documentation links and support
>   alias before release trains move forward.
...
- When legacy `hermescloud` links pop up in docs or configs, run
  ...
  checks.
- Authentication support copy is also guarded by the CLI. The
  `docs-auth-*` rules rewrite legacy `authjs.dev` references to
  [docs.hermes.chat](https://docs.hermes.chat) (and the derived locale mirrors)
  so engineering time is not wasted on manual sweeps.
