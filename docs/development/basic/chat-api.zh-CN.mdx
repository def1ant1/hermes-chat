---
title: Hermes Chat API 前后端交互逻辑
---

# Hermes Chat API 前后端交互逻辑

本文档说明了 Hermes Chat API 在前后端交互中的实现逻辑，包括事件序列和涉及的核心组件。
...
AgentRuntime 是 Hermes Chat 中的一个核心抽象层，它封装了与不同 AI 模型提供商交互的统一接口。其主要职责和特点包括：
...
5. **适配器模式**：在内部使用适配器模式，将不同提供商的 API 适配到统一的 `src/libs/agent-runtime/BaseAI.ts` 接口：
   ...
   **适配器实现示例**：
   ...
   1. **OpenRouter 适配器**：OpenRouter 是一个统一 API，可以通过它访问多个模型提供商的 AI 模型。Hermes Chat 通过适配器实现对 OpenRouter 的支持：
      ...
      class LobeOpenRouterAI implements LobeRuntimeAI {
        ...
        async chat(payload: ChatCompletionCreateParamsBase, options?: RequestOptions) {
          // 将 Hermes Chat 的请求格式转换为 OpenRouter 格式
          // 处理模型映射、消息格式等
   ...
   2. **Google Gemini 适配器**：Gemini 是 Google 的大语言模型，Hermes Chat 通过专门的适配器支持 Gemini 系列模型：
