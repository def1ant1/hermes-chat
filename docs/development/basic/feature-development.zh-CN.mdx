---
title: Hermes Chat 功能开发完全指南
---

# Hermes Chat 功能开发完全指南

本文档旨在指导开发者了解如何在 Hermes Chat 中开发一块完整的功能需求。

> \[!NOTE]
> 由于兼容历史版本，目前 TypeScript 接口仍保留 `Lobe*` 前缀（例如 `LobeAgentConfig`）。待 SDK 发布 Hermes 命名的别名后，再统一替换代码与文档中的命名。

我们将以 [RFC 021 - 自定义助手开场引导](https://github.com/hermeslabs/hermes-chat/discussions/891) 为例，阐述完整的实现流程。

## 一、更新 schema

hermes-chat 使用 postgres 数据库，浏览器端本地数据库使用 [pglite](https://pglite.dev/)（wasm 版本 postgres）。项目还使用了 [drizzle](https://orm.drizzle.team/) ORM 用来操作数据库。
...
hermes-chat 使用 [zustand](https://zustand.docs.pmnd.rs/getting-started/introduction) 作为全局状态管理框架，对于状态管理的详细实践介绍，可以查阅 [📘 状态管理最佳实践](/zh/docs/development/state-management/state-management-intro)。
...
- [ant-design](https://ant.design/) 和 [lobe-ui：](https://github.com/hermeslabs/lobe-ui)组件库
...
- [react-i18next](https://github.com/i18next/react-i18next) 和 [lobe-i18n](https://github.com/hermeslabs/lobe-cli-toolbox/tree/master/packages/lobe-i18n) ：i18n 框架和多语言自动翻译工具

hermes-chat 是个国际化项目，新加的文案需要更新默认的 `locale` 文件： `src/locales/default/setting.ts` 。
...
以上就是 Hermes Chat 开场设置功能的完整实现流程。开发者可以参考本文档进行相关功能的开发和测试。
