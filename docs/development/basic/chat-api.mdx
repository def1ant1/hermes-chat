---
title: Hermes Chat API Client-Server Interaction Logic
---

# Hermes Chat API Client-Server Interaction Logic

This document explains the implementation logic of Hermes Chat API in client-server interactions, including event sequences and core components involved.
...
AgentRuntime is a core abstraction layer in Hermes Chat that encapsulates a unified interface for interacting with different AI model providers. Its main responsibilities and features include:
...
5. **Adapter Pattern**: Internally, it uses the adapter pattern to adapt different provider APIs to the unified `src/libs/agent-runtime/BaseAI.ts` interface:
   ...
   **Adapter Implementation Examples**:
   ...
   1. **OpenRouter Adapter**: OpenRouter is a unified API that allows access to AI models from multiple providers. Hermes Chat implements support for OpenRouter through an adapter:
      ...
      class LobeOpenRouterAI implements LobeRuntimeAI {
        ...
        constructor(options: OpenAICompatibleOptions) {
          ...
          this.client = new OpenAI({
            ...
            defaultHeaders: {
              'HTTP-Referer': 'https://github.com/hermeslabs/hermes-chat',
              'X-Title': 'Hermes Chat',
            },
          });
        ...
        async chat(payload: ChatCompletionCreateParamsBase, options?: RequestOptions) {
          // Convert Hermes Chat request format to OpenRouter format
   ...
   2. **Google Gemini Adapter**: Gemini is Google's large language model. Hermes Chat supports Gemini series models through a dedicated adapter:
