...
describe('getCanonicalUrl', () => {
  ...
  it('should return correct URL for production environment', async () => {
    ...
    expect(result).toBe('https://hermes.chat/path/to/page');
    expect(urlJoin).toHaveBeenCalledWith('https://hermes.chat', 'path', 'to', 'page');
  });
  ...
  it('should work correctly without additional path arguments', async () => {
    ...
    expect(result).toBe('https://hermes.chat');
    expect(urlJoin).toHaveBeenCalledWith('https://hermes.chat');
  });
});
